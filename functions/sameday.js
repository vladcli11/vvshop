...
exports.generateAwb = functions.https.onCall(async (data) => {
  try {
    ...
    const errorData = err.response?.data || err.message || "Eroare necunoscută";
...
exports.saveAwbLabel = functions.https.onCall(async (data) => {
  const { awbNumber } = data;

  if (!awbNumber) {
    throw new functions.https.HttpsError("invalid-argument", "awbNumber lipsă");
  }
  ...
  try {
    ...
    console.error("❌ Eroare la salvarea AWB PDF:", err.response?.data || err);
